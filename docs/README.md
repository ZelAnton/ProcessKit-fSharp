...
| [Coming from ProcessKit-rs](from-rust.md) | The Rust-to-.NET vocabulary map: verbs, ownership, errors, cancellation, packages, encoding, PTY, supervision, and testing seams |
...
| [JSONL reports](jsonl-reports.md) | The opt-in, AOT-safe `ReportJson` serializer: `ProcessResult`/`Outcome`, `ProcessGroupStats`/`RunProfile`, and `MemberInfo` as self-describing JSONL lines, with a stable `"kind"` per shape, `null` for unavailable metrics, and no captured stdout/stderr/argv/environment on the wire |
