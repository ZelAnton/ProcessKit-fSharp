...
| [Running commands](commands.md) | The `Command` builder end to end: args, env, stdin sources, encodings, buffer policies, line handlers, timeouts, retry — and every consuming verb (`Run`, `OutputString`, `Probe`, …) with its error semantics |
...
task {
    ...
    | Ok group ->
        use group = group
        let! _server = group.Start(Command.create "dev-server")
