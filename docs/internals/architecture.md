...
11. `SoftStopScope.fs` — how far a soft stop reaches on one live group, read before a signal is attempted.
12. `Signal.fs` — portable signal model.
13. `ShutdownReport.fs` — what one graceful teardown actually observed: soft-signal fate, member counts, drain or escalation, elapsed time.
14. `CgroupCpuMax.fs` — cgroup v2 `cpu.max` quota formatting and parsing.
15. `Limits.fs` — resource and process-group options.
16. `StableIdentifiers.fs` — the single spelling of every union case's stable machine identifier, generated into `spec/identifiers.json`.
17. `ProcessResult.fs` — captured result.
18. `TryParser.fs` — .NET try-parse adapter.
19. `OutputPolicy.fs` — buffered and streaming overflow policies.
20. `OutputEvent.fs` — stdout/stderr event model.
21. `Finished.fs` — finish result.
22. `Stats.fs` — group/run statistics and samplers.
23. `Stdin.fs` — stdin source model.
24. `PostKillReap.fs` — bounded post-kill reap budget and the ledger that adopts an unfinished wait.
25. `PostExitDrain.fs` — bounded post-exit output drain and the severable read end it cuts.
...
- `AdoptByPid` is the same containment from a **bare pid** — no `Process`, no handle — with a different IDENTITY contract, which is why it is its own member rather than a second caller of `Adopt`. A pid is an address, not a handle, so each backend binds the group to an anchor of its own for whatever the number currently names: the process **object** behind one `OpenProcess` (Job Object), kernel cgroup membership plus a `/proc/<pid>/stat` start-time read on either side of the `cgroup.procs` write (cgroup v2), or the pid **plus** a start-time token re-read before every later probe, signal, suspend/resume and teardown kill (POSIX process group). The process-group backend therefore SUPPORTS this verb although it cannot `Adopt`: it contains by tracking rather than by moving, and an anchored number is safe to track — at the documented cost that only the adopted process itself is contained, never the processes it forks afterwards. A host with no start-time reader (the BSDs) has no anchor to take and returns `ProcessError.Unsupported`; a per-process failure (a pid naming nothing, an unreadable identity, a denied open/write, a refused assign, a target this process may not signal, or a number that changed hands during the call — rolled back out of the cgroup where that applies) is a typed `ProcessError.Adopt`. That signalability check is explicit only on the process-group backend (`Native.Posix.ensureAdoptedSignalable`, a `kill(pid, 0)` whose `EPERM` is a refusal): reading a start-time anchor proves identifiability, not control — on Linux `/proc/<pid>/stat` is world-readable — while the other two mechanisms already refuse the same case through their denied `OpenProcess`/`cgroup.procs` write. Ownership is `Adopt`'s: never `waitpid`ed, never `killpg`ed. `pid <= 0` and this process's own pid are refused by `ProcessGroup.AdoptByPid` itself, before any backend sees them.
...
A process adopted by bare pid (`ProcessGroup.AdoptByPid`) is held in a **second ledger** alongside the tracked pgids, and every difference between the two is an ownership difference. Its entry carries the start-time anchor captured at adoption as a plain token rather than an option: this backend refuses to adopt a pid whose identity it cannot read, so there is no "unknown token → fall back to the by-number verdict" case here — unlike a tracked leader of ours, whose unreaped-child slot holds its number, nothing holds an adopted foreign number, so an unreadable identity is fail-closed and delivers nothing. Delivery is always to the exact pid (`Native.Posix.signalAdopted`/`adoptedStillOurs`), never `killpg`: that process group belongs to whoever started the process, and sweeping it would reach members nobody adopted. Nothing in that ledger is ever `waitpid`ed — it is not our child — so teardown kills those pids and drains the ledger without touching the reap path, and the graceful stop snapshots it up front exactly as it snapshots the pgid identity tokens, re-reading each anchor per phase. The containment this buys is narrower than the other two mechanisms give and is documented as such: the adopted process is listed, signalled, suspended/resumed and killed with the group, but the processes it forks afterwards are not, because no POSIX primitive moves a foreign, already-`exec`ed process into our process group — and no `setpgid` is attempted on it, which would only change where a terminal's job-control signals reach it.
...
`Native.Posix` maintains a process-wide pending-wait registry keyed by PID. Linux 5.4+ registers per-child pidfds with one shared epoll reaper, while macOS registers `EVFILT_PROC` / `NOTE_EXIT` filters with one shared kqueue reaper. Older Linux and other POSIX hosts lazily install one managed `SIGCHLD` registration whose handler performs non-blocking `waitpid(..., WNOHANG)` scans. A detached leader joins this same registry after spawn; it has no separate timer or polling thread. A per-child native registration failure falls back to one blocking `waitpid`, not repeated probes. Every path resolves the exact pending-wait generation once; `reapLeader` uses a short bounded non-blocking retry loop for teardown, avoiding a permanently blocked teardown thread if a child is stuck in uninterruptible kernel sleep.
...
`CgroupBackend` is selected only on Linux when resource limits are requested. `Native.Cgroup` probes `/sys/fs/cgroup` and the hybrid `/sys/fs/cgroup/unified`, requiring a non-empty `cgroup.controllers`. Before selecting the backend, `MechanismSelection` also rejects an affinity request with `ProcessError.Unsupported` when that file does not advertise `cpuset`; this is the same missing-`cpuset.cpus` prerequisite exposed by `CapabilityProbe`. Creation then enables the required `memory`, `pids`, `cpu`, `cpuset`, and/or `io` controllers and writes `memory.max`, `pids.max`, `cpu.max`, `cpuset.cpus`, and/or `io.max`. `UpdateLimits` runs directly against the already-selected backend and rewrites those same controller files in place (enabling any controller the new caps newly need, resetting a now-`None` dimension to that controller's own "unbounded" sentinel where its file already exists) to replace the caps on the live cgroup without recreating it. A delegation or limit-file failure there becomes `ProcessError.ResourceLimit`. Each limit file's prior content is captured just before it is overwritten, and a mid-sequence write failure best-effort restores the files already changed, so a failed update leaves the live cgroup on the previous set rather than a silent mix.
...
Hard kill first writes `1` to `cgroup.kill` (kernel 5.14+). If unavailable, it best-effort freezes the cgroup and repeatedly SIGKILLs current members (up to 50 sweeps). Both paths then retry the thaw and read `cgroup.freeze` back a bounded number of times before a reusable `KillAll`/`Signal.Kill` returns `Ok`. Each fallback SIGKILL goes through the same identity-safe choke as the per-member signal sweep (pin the task with a pidfd, re-read `cgroup.procs` to reconfirm membership, then send through the pinned handle), because freezing stops members forking but not exiting, so a raw by-number kill could still land on a process that recycled a member's pid; a pid that is gone or no longer a member is skipped, and the drain check decides whether the tree died. A delivery failure is reported as `ProcessError.Io` only when the cgroup is still populated (or unreadable) afterwards, and a kernel without pidfd (below 5.3) yields that honest error rather than a downgrade to the raw kill. Either hard-kill path reports `ProcessError.Io` when the freezer still reports `1` or cannot be read; an already-unfrozen or removed freezer remains best-effort success. `HardRelease` ignores that reusable-state error while it kills/reaps every directly spawned POSIX leader and makes a bounded directory-reclaim attempt, recording a cgroup it could not remove. Resource-limit requests do not fall back to an unbounded process group when cgroup creation or delegation fails.
...
`Pump.MeteredStream` is what makes `RunningProcess.StdoutBytesSeen`/`StderrBytesSeen` honest: `OutputSessions` wraps each of the handle's two parent read ends in one, and every read below it — the line loops, the raw-text loop, the byte-chunk loop, the discard drains, the raw capture, `ContentLengthSession`'s framed reader — is therefore counted at the moment the OS handed the bytes back, before any decoding, framing or retention policy. Counting at the stream rather than at each read site is what makes the counter cover every mode by construction (including a readiness probe's background drain, which is handed the same metered stream) and keeps a pump added later from escaping it; it is the same shape `Timeouts.ActivityStream` already uses to observe idle-timeout activity, and it sits outside `SeverableStream`, so a severed read's synthetic EOF adds nothing. These counters are deliberately a different quantity from `LineBuffer`'s retained totals above, and they are what a fail-loud streaming overflow (`StreamFullMode.Error` on the line and merged-event channels) quotes as its `OutputTooLarge` byte total; the chunk channels keep their own queued-bytes counters, which is the quantity their capacity actually bounds.
...
`Command.LaunchDetached` is the one deliberate exception to that sequence, and it is structured so it can never dilute it. It has its own spawn path in each platform layer (`Native.Windows.spawnDetachedWindows`, `Native.Posix.spawnDetachedPosix`) which skips the whole transaction: no Job assignment (so no `CREATE_SUSPENDED`/resume dance), `POSIX_SPAWN_SETSID` instead of the tracked process group, no `Track`, no retained handle, and no `Release`. On POSIX, preparation initializes the shared exit-notification mechanism before `posix_spawn`; after the start-time snapshot, the direct leader is atomically registered in the same pending-wait ledger as an ordinary child. That ledger alone performs the final reap through pidfd/epoll, kqueue, shared `SIGCHLD`, or a single blocking fallback after a registration failure — never a timed polling loop. A failed preparation prevents spawn, while a failed handoff synchronously kills and reaps the fresh session before returning `ProcessError.Spawn`. It returns only a `DetachedProcess` (pid + start time), never a `RunningProcess`, so the background wait owner does not become a public lifetime/control handle, and every builder knob that would require the parent-side machinery it does not create is refused up front with a typed `ProcessError.Unsupported` (`DetachedLaunch.incompatibleKnob` in `CommandVerbs.fs`). It shares the resolution, command-line, environment, stdio-opening, privilege-drop, and exit-wait helpers with the contained paths rather than re-deriving them, so the divergence is exactly the containment and private exit-status collection, nothing else. Keep it that way: a detached mode reachable as a *flag* on the ordinary path would make the kill-on-drop guarantee conditional everywhere, which is precisely what the separate verb avoids.
...
| Detached launch (`LaunchDetached`) | Created running, assigned to no Job, both handles closed at once | `POSIX_SPAWN_SETSID`: own session, no pgid tracking; the shared event-driven wait ledger owns the direct leader's status after handoff | Same as the middle column — a detached child joins no cgroup; its direct leader uses the shared wait ledger |
...
| Post-run limit evidence (`LimitEvidence`) | `Unknown` for every axis ever capped (no post-mortem record); `NotTripped` for one never capped | `Unknown` on every axis, unconditionally — no evidence apparatus at all | Real per-axis evidence (`memory.events`/`pids.events`/`cpu.stat`); `Unknown` only when a counter can't be read |
