...
its liveness probes inherit the same clock automatically.
...
- `WithCommandProjection((program, args) -> (program, args))` — project the **persisted** command
  ...
  program and its (normalized) arguments, plus the `Command.Arg0` override (never projected, since
  this hook covers only `Program`/`Args`) — taken *before* the projection runs and stored beside the
